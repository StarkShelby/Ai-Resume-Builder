    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-16">
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-6 pt-28 pb-20 bg-white/80 backdrop-blur-sm rounded-xl shadow-lg my-10 grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div className="lg:col-span-1 flex flex-col items-center lg:sticky lg:top-24 lg:self-start h-fit">
          ...
          {/* PREVIEW */}
          <div className="bg-white p-4 rounded-2xl border border-gray-200 shadow-md min-h-[500px] w-full">
            <h3 className="text-2xl font-bold text-gray-800 mb-4 border-b pb-2">
              Resume Preview
            </h3>
            {loading && !savedResume && !fileURL && (
              <p className="text-gray-500 text-center">Loading resume...</p>
            )}
            {savedResume ? (
              renderSavedPreview()
              ...
              <div className="rounded-xl overflow-hidden border">
                ...
                {uploadedFile.type.startsWith("image/") && (
                  <img src={fileURL} className="w-full" alt="Resume Preview" />
                )}
              </div>
            ) : (
              <p className="text-gray-400 text-center">
                {resumeId
                  ? "Resume details not found or failed to load."
                  : "Upload a resume or select from dashboard to see a preview."}
              </p>
            )}
          </div>
        </div>
